      ...
      <div class="card mt-6 p-4">
        ...
        <div v-if="simulationLog.length > 0">
          <div v-for="entry in simulationLog" :key="entry.timestamp" class="log-entry">
            ...
            <p v-if="entry.alerts && entry.alerts.length > 0"><strong>Alerts:</strong> {{ entry.alerts.length }} (e.g., {{ entry.alerts[0].message }})</p>
            <p v-else><strong>Alerts:</strong> None</p>
...
// Function to generate random-ish dummy data
const generateDummyData = () => {
  ...
  const data = {
    ...
    breed_type: ['Normal Breed', 'Cross Breed', 'Holstein', 'Jersey', 'Indigenous Breed', 'Other'][Math.floor(Math.random() * 6)], // Added more breeds
